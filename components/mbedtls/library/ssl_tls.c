...
int mbedtls_ssl_derive_keys( mbedtls_ssl_context *ssl )
{
    ...
    if( session->compression == MBEDTLS_SSL_COMPRESS_DEFLATE )
    {
        if( ssl->compress_buf == NULL )
        {
            ...
            ssl->compress_buf = mbedtls_calloc( 1, ssl->buffer_len );
            if( ssl->compress_buf == NULL )
            {
...
static int ssl_decrypt_buf( mbedtls_ssl_context *ssl )
{
    ...
    defined(MBEDTLS_SSL_PROTO_TLS1_2)
        if( ssl->minor_ver > MBEDTLS_SSL_MINOR_VERSION_0 )
        {
            ...
            correct &= ( padding_idx < ssl->max_content_len +
                                       ssl->transform_in->maclen );
...
static int ssl_compress_buf( mbedtls_ssl_context *ssl )
{
    ...
    ssl->transform_out->ctx_deflate.avail_out = ssl->buffer_len;
...
static int ssl_decompress_buf( mbedtls_ssl_context *ssl )
{
    ...
    ssl->transform_in->ctx_inflate.avail_out = ssl->max_content_len;
    ...
    ssl->in_msglen = ssl->max_content_len -
...
int mbedtls_ssl_fetch_input( mbedtls_ssl_context *ssl, size_t nb_want )
{
    ...
    if( nb_want > ssl->buffer_len - (size_t)( ssl->in_hdr - ssl->in_buf ) )
    {
    ...
    if( ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM )
    {
        ...
        {
            len = ssl->buffer_len - ( ssl->in_hdr - ssl->in_buf );
...
static int ssl_reassemble_dtls_handshake( mbedtls_ssl_context *ssl )
{
    ...
    if( ssl->handshake->hs_msg == NULL )
    {
        ...
        if( ssl->in_hslen > ssl->max_content_len )
        {
...
static int ssl_handle_possible_reconnect( mbedtls_ssl_context *ssl )
{
    ...
    ret = ssl_check_dtls_clihlo_cookie(
            ...
            ssl->out_buf, ssl->max_content_len, &len );
...
static int ssl_parse_record_header( mbedtls_ssl_context *ssl )
{
    ...
    if( ssl->in_msglen > ssl->buffer_len
...
#if defined(MBEDTLS_SSL_PROTO_SSL3)
        if( ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_0 &&
            ssl->in_msglen > ssl->transform_in->minlen + ssl->max_content_len )
        {
            MBEDTLS_SSL_DEBUG_MSG( 1, ( "bad message length" ) );
            return( MBEDTLS_ERR_SSL_INVALID_RECORD );
        }
...
static int ssl_prepare_record_content( mbedtls_ssl_context *ssl )
{
    ...
    if( !done && ssl->transform_in != NULL )
    {
        ...
        if( ssl->in_msglen > ssl->max_content_len )
        {
            MBEDTLS_SSL_DEBUG_MSG( 1, ( "bad message length" ) );
            return( MBEDTLS_ERR_SSL_INVALID_RECORD );
        }
...
int mbedtls_ssl_write_certificate( mbedtls_ssl_context *ssl )
{
    ...
    {
        n = crt->raw.len;
        if( n > ssl->max_content_len - 3 - i )
        {
            MBEDTLS_SSL_DEBUG_MSG( 1, ( "certificate too large, %d > %d",
                           i + 3 + n, ssl->max_content_len ) );
                       ...
                       const mbedtls_ssl_config *conf )
{
    ...
    if (conf->max_content_len > 0) {
        ssl->max_content_len = conf->max_content_len;
    } else {
        ssl->max_content_len = MBEDTLS_SSL_MAX_CONTENT_LEN;
    }

    ssl->buffer_len = ssl->max_content_len
     ...
     */
    if( ( ssl-> in_buf = mbedtls_calloc( 1, ssl->buffer_len  ) ) == NULL ||
        ( ssl->out_buf = mbedtls_calloc( 1, ssl->buffer_len  ) ) == NULL )
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "alloc(%d bytes) failed", ssl->buffer_len  ) );
...
static int ssl_session_reset_int( mbedtls_ssl_context *ssl, int partial )
{
    ...
    memset( ssl->out_buf, 0, ssl->buffer_len );
    if( partial == 0 )
        memset( ssl->in_buf, 0, ssl->buffer_len );
...
void mbedtls_ssl_conf_max_content_len( mbedtls_ssl_config *conf,
    ...
    conf->max_content_len = max_content_len;
}
                ...
                const unsigned char *psk_identity, size_t psk_identity_len )
{
    ...
    if( ( psk_identity_len >> 16 ) != 0 ||
        psk_identity_len > ssl->max_content_len )
    {
        return( MBEDTLS_ERR_SSL_BAD_INPUT_DATA );
    }
...
void mbedtls_ssl_free( mbedtls_ssl_context *ssl )
{
    ...
    if( ssl->out_buf != NULL )
    {
        mbedtls_zeroize( ssl->out_buf, ssl->buffer_len );
        mbedtls_free( ssl->out_buf );
    }

    if( ssl->in_buf != NULL )
    {
        mbedtls_zeroize( ssl->in_buf, ssl->buffer_len );
        mbedtls_free( ssl->in_buf );
    }
    ...
    if( ssl->compress_buf != NULL )
    {
        mbedtls_zeroize( ssl->compress_buf, ssl->buffer_len );
